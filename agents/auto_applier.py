...
def apply_greenhouse(page, url: str, info: dict, resume_path: str, cover_letter: str) -> bool:
    ...
    apply_btn = page.query_selector('a[href*="#app"], button:has-text("Apply"), a:has-text("Apply")')
    if apply_btn:
        apply_btn.click()
        page.wait_for_timeout(2000)
    ...
    name_fields = [
        ...
        ('input[name*="last_name"]', " ".join(info.get("name", "").split()[1:])),
    ...
    # Upload resume
    resume_input = page.query_selector('input[type="file"][name*="resume"], input[type="file"][id*="resume"]')
    if not resume_input:
    ...
    location_el = page.query_selector('input[name*="location"], input[placeholder*="location"]')
    if location_el:
...
def apply_workday(page, url: str, info: dict, resume_path: str, cover_letter: str) -> bool:
    ...
    all_inputs = page.query_selector_all('input[type="text"]:visible, input[type="email"]:visible, input[type="tel"]:visible')
...
def auto_apply(
        ...
        with sync_playwright() as p:
            ...
            if platform == "greenhouse":
                ...
                if dry_run:
                    ...
                    attempt.screenshot_path = str(screenshot_dir / f"dryrun_greenhouse_{ts}.png")
                    page.screenshot(path=attempt.screenshot_path, full_page=True)
                    success = True
                else:
                    success = apply_greenhouse(page, job_url, info, resume_path, cover_letter_text)
                ...
                if dry_run:
                    ...
                    success = apply_workday(page, job_url, info, resume_path, cover_letter_text)
                ...
                if dry_run:
                    ...
                    success = apply_generic(page, job_url, info, resume_path, cover_letter_text)
    ...
    if attempt.success:
        ...
        print(f"   ✅ [{mode}] {company} — {title}")
    else:
        print(f"   ⚠️  Could not auto-apply: {attempt.error or 'form submission unclear'}")
