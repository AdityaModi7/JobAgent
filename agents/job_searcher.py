...
def is_entry_level_friendly(title: str, description: str = "") -> bool:
    ...
    has_entry_signal = any(signal in combined for signal in ENTRY_LEVEL_SIGNALS)
...
def search_linkedin(keyword: str, location: str = None, max_results: int = 25) -> list[JobSearchResult]:
    ...
    try:
        ...
        for card in job_cards[:max_results]:
            title_el = card.select_one(".base-search-card__title, .base-card__full-link")
        ...
        print(f"   ⚠️  LinkedIn search failed for '{keyword}': {e}")
...
def search_indeed(keyword: str, location: str = None, max_results: int = 25) -> list[JobSearchResult]:
    ...
    try:
        ...
        job_cards = soup.select(".job_seen_beacon, .jobsearch-ResultsList > li, .resultContent")
...
def run_job_search(filters: SearchFilters = None) -> SearchRun:
    ...
    print(f"   Experience: {filters.experience_level} (max {filters.max_experience_years} yrs)")
    ...
    if filters.exclude_companies:
        ...
        print(f"   Excluded companies: removed {before - len(unique_results)} results")
    ...
    if removed:
        print(f"   Salary filter (>= ${filters.min_salary:,}): removed {removed} results")
    ...
    print(f"\n✅ Search complete: {len(unique_results)} ML/AI roles found")
