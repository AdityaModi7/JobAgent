...
def run_workflow(config: WorkflowConfig = None) -> dict:
    ...
    if not search_run.results:
        print("\n⚠️  No jobs found matching criteria. Try broadening search filters.")
        return summary
    ...
    for i, job_result in enumerate(search_run.results):
        if applied_count >= config.max_applications_per_run:
            print(f"\n🛑 Reached max applications ({config.max_applications_per_run}). Stopping.")
            break

        # Skip if already applied
        app_key = f"{job_result.company.lower()}|{job_result.title.lower()}"
        if app_key in already_applied:
            print(f"\n[{i+1}] ⏭️  Already applied: {job_result.title} at {job_result.company}")
            continue
        ...
        try:
            if job_result.url:
                ...
                print(f"   ⚠️  No URL or description — skipping")
        ...
        try:
            ...
            if fit.overall_score < config.min_fit_score:
                print(f"   ⚠️  Fit score {fit.overall_score} below threshold {config.min_fit_score}. Skipping.")
...
def _print_summary(summary: dict, dry_run: bool):
    ...
    if dry_run:
        print("\n💡 This was a DRY RUN. To submit applications for real, run:")
